...
def configure_api(app:Flask, gt:Git):
    ...
    def commits():
        ...
        print(authors)
        return jsonify({"authors":list(authors), "commits": list(map(lambda x: {'message': x.message, 'committed_datetime': x.committed_datetime, 'author': str(x.committer)}, commits))})
